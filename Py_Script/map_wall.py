...
class WallMapping:
    ...
    def update_bed_size(self):
        if len(self.corner_points) == 2:
            x_bed_size = abs(self.corner_points[1][0] - self.corner_points[0][0])
            y_bed_size = abs(self.corner_points[1][1] - self.corner_points[0][1])
            self.robot_control.send_gcode(f"M211 S1 X{x_bed_size} Y{y_bed_size}")
