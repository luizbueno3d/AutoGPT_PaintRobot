...
from PyQt6.QtWidgets import QApplication, QMainWindow, QWidget, QPushButton, QLabel, QGridLayout, QVBoxLayout, QSlider, QLineEdit, QMessageBox, QSizePolicy, QComboBox, QScrollArea
...
class MainWindow(QMainWindow):
    def __init__(self, robot_control):
        ...
        self.distance_input = QLineEdit() # Input distance field to Move X,Y,Z on GUI.
        self.distance_input.setFixedWidth(160)
        self.distance_input.setPlaceholderText('Distance (in mm)') #distance in cm as Ghost text:
        self.grid_layout.addWidget(self.distance_input, 3, 0)


        # Create labels to display the points
        self.point1_label = QLabel(self)
        self.grid_layout.addWidget(self.point1_label, 0, 5)

        self.point2_label = QLabel(self)
        self.grid_layout.addWidget(self.point2_label, 1, 5)

        # Create buttons to clear the points
        self.clear_point1_button = QPushButton('Clear Point 1', self)
        self.clear_point1_button.clicked.connect(self.on_clear_point1_button_click)
        self.grid_layout.addWidget(self.clear_point1_button, 0, 6)

        self.clear_point2_button = QPushButton('Clear Point 2', self)
        self.clear_point2_button.clicked.connect(self.on_clear_point2_button_click)
        self.grid_layout.addWidget(self.clear_point2_button, 1, 6)

        # Create "Calculate Map G-code" button
        self.calculate_map_gcode_button = QPushButton('Calculate Map G-code', self)
        # Connect the button to the map_manually_and_generate_gcode method
        self.calculate_map_gcode_button.clicked.connect(self.wall_mapping.map_manually_and_generate_gcode)
        # Add the button to the grid layout
        self.grid_layout.addWidget(self.calculate_map_gcode_button, 1, 7)


        # Create QLineEdit widgets for # spray_jet_width and spray_jet_overlap #
        self.spray_jet_width_input = QLineEdit(self)
        self.spray_jet_width_input.setPlaceholderText('200')
        self.grid_layout.addWidget(self.spray_jet_width_input, 2, 6)
        self.grid_layout.addWidget(QLabel('mm - Spray Jet Width'), 2, 7)


        self.spray_jet_overlap_input = QLineEdit(self)
        self.spray_jet_overlap_input.setPlaceholderText('10')
        self.grid_layout.addWidget(self.spray_jet_overlap_input, 3, 6)
        self.grid_layout.addWidget(QLabel('mm - Spray Jet Overlap'), 3, 7)
        ...
        self.zero_points_button = QPushButton('X0,Y0', self)
        self.zero_points_button.setFixedWidth(50)
        self.zero_points_button.clicked.connect(self.on_zero_points_button_click)
        self.grid_layout.addWidget(self.zero_points_button, 5, 2)
        ...
        # Display G-code created from "Calculate Map G-code"
        self.gcode_display = QLabel("G-code:")
        self.grid_layout.addWidget(self.gcode_display, 7, 5, 1, 3)  # Adjust the position as needed
        ...
        #function to clear 1st point from Map Wall
    def on_clear_point1_button_click(self):
        self.wall_mapping.corner_points[0] = None
        self.point1_label.setText('')

        #function to clear 2nd point from Map Wall
    def on_clear_point2_button_click(self):
        if len(self.wall_mapping.corner_points) > 1:
            self.wall_mapping.corner_points[1] = None
        self.point2_label.setText('')


    def on_zero_points_button_click(self):
        self.robot_control.send_gcode("G92 X0 Y0")
    ...
    def initialize_robot_control(self):
        try:
            ...
            time.sleep(1)  # Add a delay of 2 seconds because for 1st Gcode.
            self.update_command_display()
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))
